            ...
            conexion.query("SELECT * FROM users WHERE user = ?", [user], async (error, results)=>{
                if(results.length == 0 || ! (await bcryptjs.compare(pass, results[0].pass )) ){
                        res.render("login", {
                            alert:true,
                            alertTitle:"advertencia",
                            alertMessage:"Sin contraseña no pasas :)",
                            alertIcon:"info",
                            showConfirmButton: true,
                            timer: false,
                            ruta: "login"
...
exports.deleteSong = (req, res) => {
  ...
  if (!nombre) {
    return res.status(400).send('Falta el nombre de la canción');
  }

  // Ajustá la ruta a donde guardás las canciones
const rutaArchivo = path.join(__dirname, '..', 'uploads', nombre);
